export async function uploadToWordPress(fileBlob: Blob, fileName: string, _fileType: string) {
  const WP_USER = process.env.WP_USER || 'coupiya_admin';
  const WP_PASSWORD = process.env.WP_PASSWORD || 'WONZ xxSG BpSa XfOg 7m03 cuzH';
  const WP_REST_URL = process.env.NEXT_PUBLIC_WP_REST_URL || 'https://api.coupiya.com/wordpress/wp-json/';

  if (!WP_PASSWORD || WP_PASSWORD === 'WONZ xxSG BpSa XfOg 7m03 cuzH') {
    throw new Error('请先在 .env.local 中配置有效的 WordPress 应用密码');
  }
